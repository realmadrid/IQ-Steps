...
import static game.TestUtility.SOLUTIONS_MULTI;
import static game.TestUtility.SOLUTIONS_ONE;
    ...
    public void testMulti() {
        for (int i = 0; i < SOLUTIONS_MULTI.length; i++) {
            ...
            for (int j = 0; j < result.length; j++) {
                if (!solset.contains(TestUtility.normalize(result[j])))
                    mismatch += " " + result[j] + " (" + TestUtility.normalize(result[j]) + ")";
            }
